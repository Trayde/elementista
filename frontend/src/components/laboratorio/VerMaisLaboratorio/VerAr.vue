...
export default {
  ...
  methods: {
    ...
    async show() {
      const obterResponse = await ApiMethodsAtividades.obterAr();
      let arrays = [];
      ...
      const response = await ApiMethodsAtividades.obterArId(arrays[0].id_atividade)
      response.map((dados) => {
        this.content = {
    ...
    async listaCards() {
      const response = await ApiMethodsAtividades.obterAr()
