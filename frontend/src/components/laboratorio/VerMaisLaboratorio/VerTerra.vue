    ...
    <div class="col-md-8  grid-margin">
      <div class="row">
        <div class="col-md-12 grid-margin ">
          ...
          <button v-if="content.isArquivo" @click="downloadFile" class="btn btn-primary mt-3 spacamento-top">Baixar
            Arquivo</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    async show() {
      const obterResponse = await ApiMethodsAtividades.obterTerra();
      let arrays = [];
      ...
      const response = await ApiMethodsAtividades.obterTerraId(arrays[0].id_atividade)
      response.map((dados) => {
        this.content = {
    ...
    async listaCards() {
      const response = await ApiMethodsAtividades.obterTerra()
